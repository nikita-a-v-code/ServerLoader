...
const createExcelWorkbook = async (data) => {
  ...
  data.forEach((item, index) => {
    ...
    // Форматирование номера дома - добавляем "дом " если введена только цифра
    const formatHouse = (house) => {
      if (!house) return null;
      const trimmed = String(house).trim();
      if (trimmed === "") return null;
      // Если значение состоит только из цифр, добавляем "дом "
      if (/^\d+$/.test(trimmed)) {
        return `дом ${trimmed}`;
      }
      return trimmed;
    ...
    // Форматирование квартиры - добавляем ", кв. " вместо просто запятой
    const formatApartment = (apartment) => {
      if (!apartment) return null;
      return `, кв. ${String(apartment).trim()}`;
    };

    // Хелпер: возвращает null вместо пустой строки, чтобы Excel оставлял ячейки пустыми
    ...
    const row = [
      ...
      formatApartment(normalizedItem.apartment),
      emptyToNull(normalizedItem.consumerName),
      emptyToNull(normalizedItem.deliveryPoint),
      emptyToNull(normalizedItem.subscriberType),
      emptyToNull(normalizedItem.accountStatus),
      emptyToNull(normalizedItem.contractNumber),
      emptyToNull(psCode),
      emptyToNull(feederCode),
      emptyToNull(tpCode),
      emptyToNull(feeder04Code),
      emptyToNull(consumerCode),
      emptyToNull(normalizedItem.transformerSubstationNumber),
      emptyToNull(normalizedItem.numberSupport04),
      emptyToNull(normalizedItem.maxPower),
      emptyToNull(normalizedItem.simCardFull),
      emptyToNull(normalizedItem.simCardShort),
      emptyToNull(normalizedItem.deviceModel),
      emptyToNull(normalizedItem.numberPhases),
      emptyToNull(normalizedItem.serialNumber),
      emptyToNull(normalizedItem.verificationDate),
      emptyToNull(normalizedItem.verificationInterval),
      emptyToNull(normalizedItem.dateInstallation),
      emptyToNull(normalizedItem.numberTerminal),
      emptyToNull(normalizedItem.numberCasing),
      emptyToNull(normalizedItem.communicatorNumber),
      emptyToNull(normalizedItem.ttType),
      emptyToNull(normalizedItem.ttSerialA),
      emptyToNull(normalizedItem.ttSerialB),
      emptyToNull(normalizedItem.ttSerialC),
      emptyToNull(normalizedItem.ttDateA),
      emptyToNull(normalizedItem.ttIntervalA),
      emptyToNull(normalizedItem.ttDateB),
      emptyToNull(normalizedItem.ttIntervalB),
      emptyToNull(normalizedItem.ttDateC),
      emptyToNull(normalizedItem.ttIntervalC),
      emptyToNull(normalizedItem.ttCoeff) || "1",
      emptyToNull(normalizedItem.ttSealA),
      emptyToNull(normalizedItem.ttSealB),
      emptyToNull(normalizedItem.ttSealC),
      emptyToNull(normalizedItem.tnType),
      emptyToNull(normalizedItem.tnSerialA),
      emptyToNull(normalizedItem.tnSerialB),
      emptyToNull(normalizedItem.tnSerialC),
      emptyToNull(normalizedItem.tnDateA),
      emptyToNull(normalizedItem.tnIntervalA),
      emptyToNull(normalizedItem.tnDateB),
      emptyToNull(normalizedItem.tnIntervalB),
      emptyToNull(normalizedItem.tnDateC),
      emptyToNull(normalizedItem.tnIntervalC),
      emptyToNull(normalizedItem.tnCoeff) || "1",
      emptyToNull(normalizedItem.tnSealA),
      emptyToNull(normalizedItem.tnSealB),
      emptyToNull(normalizedItem.tnSealC),
      emptyToNull(normalizedItem.note),
      emptyToNull(normalizedItem.networkAddress),
      emptyToNull(normalizedItem.ipAddress),
      emptyToNull(normalizedItem.password),
      emptyToNull(normalizedItem.advSettings),
      emptyToNull(normalizedItem.comPorts),
      emptyToNull(normalizedItem.port),
      emptyToNull(normalizedItem.nameConnection),
      emptyToNull(normalizedItem.finalCoeff),
      emptyToNull(normalizedItem.requests),
      emptyToNull(normalizedItem.protocol),
      emptyToNull(normalizedItem.nameUSPD),
      emptyToNull(normalizedItem.typeUSPD),
      emptyToNull(normalizedItem.numberUSPD),
      emptyToNull(normalizedItem.userUSPD),
      emptyToNull(normalizedItem.passwordUSPD),
    ];
